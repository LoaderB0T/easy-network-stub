...
describe('Parameters', () => {
  ...
  test('Number param', async () => {
    ...
    const response2 = await parseFetch(fakeNetwork, {
      method: 'GET',
      url: 'MyServer/api/Blog/posts/648354',
    });
    expect(response2.id).toBe(648354);
    // String in param will fail
    await parseFetch(fakeNetwork, { method: 'GET', url: 'MyServer/api/Blog/posts/12a' }).catch(
      e => e
    );
    expect(testEasyNetworkStub.lastError.message).toBe(
      'Route not mocked: [GET] MyServer/api/Blog/posts/12a'
    );
  });
  ...
  test('Boolean param', async () => {
    ...
    expect(testEasyNetworkStub.lastError.message).toBe(
      'Route not mocked: [GET] MyServer/api/Blog/posts/notBool'
    );
